...
def parse_pairs_txt(file_path):
    sets_data = []  # To store all the data sets

    with open(file_path, 'r') as file:
        # Read the first line
        first_line = file.readline().strip()
        num_sets, num_pairs_per_set = map(int, first_line.split())
        for _ in range(num_sets):
            ...
            for _ in range(num_pairs_per_set):
                ...
                set_data['matched_pairs'].append({
                    'name': name,
                    'image1': f"{name}_{int(n1):04d}.jpg",
                    'image2': f"{name}_{int(n2):04d}.jpg"
                })
            
            # Read mismatched pairs
            for _ in range(num_pairs_per_set):
                line = file.readline().strip()
                name1, n1, name2, n2 = line.split()
                set_data['mismatched_pairs'].append({
                    'name1': name1,
                    'image1': f"{name1}_{int(n1):04d}.jpg",
                    'name2': name2,
                    'image2': f"{name2}_{int(n2):04d}.jpg"
                })

            # Add the current set to the sets_data list
...
if __name__ == "__main__":
    ...
    file_path = "/mnt/afs/machengqian/data/face/lfw/pairs.txt"
    image_folder_path = "/mnt/afs/machengqian/data/face/lfw"
    result_path = args.result
    # result_path = "/mnt/afs/machengqian/mllm/qwen2vl/results/lfw_qwen2_5vl_7b_grpo_0313_0_kl_cot_v1_t_0.json"
    ...
    for i in range(len(pairs)):
        for matched_pair in pairs[i]["matched_pairs"]:
            start = time.time()
            image1 = encode_image(os.path.join(image_folder_path, matched_pair["name"], matched_pair["image1"]))
            image2 = encode_image(os.path.join(image_folder_path, matched_pair["name"], matched_pair["image2"]))

            response = run_multi_image(model, question, image1, image2, args.temp)
            print(response)
            result[f"{matched_pair['name']}_{matched_pair['image1']}_{matched_pair['image2']}"] = [response, "Yes"]
        ...
        for mismatched_pair in pairs[i]["mismatched_pairs"]:
            start = time.time()
            image1 = encode_image(os.path.join(image_folder_path, mismatched_pair["name1"], mismatched_pair["image1"]))
            image2 = encode_image(os.path.join(image_folder_path, mismatched_pair["name2"], mismatched_pair["image2"]))
            # if args.cot:
            #     question = QUESTION_TEMPLATE.format(Question=question)
            response = run_multi_image(model, question, image1, image2, args.temp)
            print(response)
            result[f"{mismatched_pair['name1']}_{mismatched_pair['image1']}_{mismatched_pair['name2']}_{mismatched_pair['image2']}"] = [response, "No"]
