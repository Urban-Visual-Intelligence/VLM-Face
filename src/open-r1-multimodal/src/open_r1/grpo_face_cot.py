...
class GRPOScriptArguments(ScriptArguments):
    ...
    reward_funcs: list[str] = field(
        ...
        # default_factory=lambda: ["accuracy", "format", "bbox_bonus_format"],
        ...
        if reward == 0.0:
            try:
                ...
                if has_numbers:
                    ...
                    reward = 2.0 if student_answer == ground_truth else 0.0
...
def bbox_format_reward(completions, **kwargs):
    ...
    if os.getenv("DEBUG_MODE") == "true":
        log_path = os.getenv("LOG_PATH")
        with open(log_path, "a", encoding='utf-8') as f:
            f.write(f"------------- {current_time} BBOX Format reward -------------\n")
            for content, match in zip(completion_contents, matches):
                f.write(f"Content: {content}\n")
                f.write(f"Has format: {bool(match)}\n")
...
def bbox_acc_bonus_reward(completions, solution, **kwargs):
    acc_reward = accuracy_reward(completions, solution)
    bbox_reward = bbox_format_reward(completions)
    
    return [1.0 if acc_match * bbox_match else 0.0 for acc_match, bbox_match in zip(acc_reward, bbox_reward)]


reward_funcs_registry = {
    "accuracy": accuracy_reward,
    "format": format_reward,
    "bbox_format": bbox_format_reward,
    "bbox_bonus_format": bbox_acc_bonus_reward
}
...
# THINK_PROMPT ="Output Format: Based on the Instruction above, output the thinking process in <think> </think> and final answer in <answer> </answer> tags."

THINK_PROMPT ="Output Format: Based on the instruction above, first output your reasoning step-by-step inside <think> </think> tag. Then, provide the final answer based on that reasoning in <answer> </answer> tags."
...
def main(script_args, training_args, model_args):
    ...
    with open(script_args.dataset_config_path, 'r') as file:
        datasets = json.load(file)
    data_files = list()
    image_folders = list()
    for key, value in datasets.items():
        data_files.append(value["annotation"])
        image_folders.append(value["root"])
        ...
        return {
            ...
            'prompt': [{
                ...
                'content': [
                    ...
                    {'type': 'text', 'text': example['problem'] + THINK_PROMPT}
                ]
            }]
        }
